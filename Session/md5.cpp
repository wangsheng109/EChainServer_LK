...
int GetMd5Resulte(unsigned char *outResult, char *input, int inputlen)
{
	   ...
	   unsigned char restule[16];
	   struct MD5Context ctx;
	   int i = 0,j=0;
	   char tmpBuf[40] = {0};
	   
	   TRACE_INFO("md5 data len:%d\n",inputlen);
   
    for(i=0;i<inputlen;i++)
    {
    	TRACE_INFO("%02x_",input[i]);
    }  
    TRACE_INFO("\n");
 ...
 //    memcpy(outResult,digest,16);    
	...
	memcpy(outResult,tmpBuf,32);   
/*    for(i=4 ; i<12 ;i++)
    {
       sprintf(restule+j*2,"%02x",digest[i]);
       j++;
    }
    memcpy(outResult,restule,16);   
*/    return 0;
}
/*****************************************************************************/
#endif
